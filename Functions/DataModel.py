...
class Gaivota(BaseModel):
    ...
    downloads_dir : str
        Directory where downloaded files are stored. By default, it is a 'downloads' folder in the current working directory.
    data_files : List[dict]
        A list of dictionaries, each containing a URL to a CSV file and its corresponding filename. These files are essential data sources for the class.
    airlines : Any
        A dictionary or DataFrame that holds data about airlines. Initially empty.
    airports : Any
        A dictionary or DataFrame that holds data about airports. Initially empty.
    airplanes : Any
        A dictionary or DataFrame that holds data about airplanes. Initially empty.
    routes : Any
        A dictionary or DataFrame that holds data about flight routes. Initially empty.
    airports_distances : Any
        A dictionary or DataFrame that holds data about distances between airports. Initially empty.

    """
    
    project_dir: str = Field(default=os.getcwd(), description="Root directory of the project.")
    downloads_dir: str = Field(default=os.path.join(os.getcwd(), 'downloads'))
    data_files: List[dict] = Field(default=[
        {'url': 'https://gitlab.com/adpro1/adpro2024/-/raw/main/Files/airlines.csv', 'filename': 'airlines.csv'},
        {'url': 'https://gitlab.com/adpro1/adpro2024/-/raw/main/Files/airplanes.csv', 'filename': 'airplanes.csv'},
        {'url': 'https://gitlab.com/adpro1/adpro2024/-/raw/main/Files/airports.csv', 'filename': 'airports.csv'},
        {'url': 'https://gitlab.com/adpro1/adpro2024/-/raw/main/Files/routes.csv', 'filename': 'routes.csv'}])
    airlines: Any = {}
    airports: Any = {}
    airplanes: Any = {}
    routes: Any = {}
    ...
    def __init__(self):
        ...
        for file_info in self.data_files:
            ...
            setattr(self, os.path.splitext(file_info['filename'])[0], df)
    ...
    def download_file(self, url: HttpUrl, file_path: str) -> None:
        ...
        # Check if the file was downloaded successfully, if not, try downloading from the ZIP archive
        if not os.path.exists(file_path):
            print(f"Trying to download {os.path.basename(file_path)} from ZIP archive...")
            ...
            extracted_file_path = os.path.join(self.downloads_dir, os.path.basename(file_path))
            if os.path.exists(extracted_file_path):
                print(f"{os.path.basename(file_path)} extracted from ZIP archive successfully.")
            else:
                print(f"Failed to extract {os.path.basename(file_path)} from ZIP archive.")
    
    def remove_columns(self):
        """Funtion to download the files online"""
        if isinstance(self.airlines, pd.DataFrame) and not self.airlines.empty:
            self.airlines = self.airlines.drop(columns=['Alias', 'Active'], errors='ignore')
        if isinstance(self.airports, pd.DataFrame) and not self.airports.empty:
            self.airports = self.airports.drop(columns=['Source', 'DST', 'Timezone'], errors='ignore')
 
    def plot_airports_by_country(self, country_name: str):
        """
        Plot all airports in a given country.
    
        This method receives a string (a country name) and plots a map of the country and all it's airports.
        ...
        country_airports = airports[
            airports['Country'].str.contains(country_name, case=False, na=False)
        ]
        ...
        gdf_airports.plot(axes=axes, marker='o', color='blue', markersize=25, label='-Airport')
    ...
    def distance_analysis(self):
        from RealDistances import calculate_distance
        """
        ...
        for _, row in self.routes.iterrows():
            ...
            # Find the latitude and longitude for both the source and destination airports
            source_info = self.airports.loc[self.airports['IATA'] == source_iata]
            destination_info = self.airports.loc[self.airports['IATA'] == destination_iata]
    ...
    def internal_airport(self, airport, show_internal=True):
        ...
        Args:
            ...
            show_internal (bool, optional): If True, show only internal flights; otherwise, show all flights. Default is True.
        ...
        source = routes[["index", "Airline", "Airline ID", "Source airport", "Source airport ID", "Codeshare", "Stops", "Equipment"]]
        destination = routes[["index", "Airline", "Airline ID", "Destination airport", "Destination airport ID", "Codeshare", "Stops", "Equipment"]]

        source_country = source.merge(airports, left_on="Source airport", right_on="IATA", how="left")
        destination_country = destination.merge(airports, left_on="Destination airport", right_on="IATA", how="left")

        source_country = source_country.iloc[:, [0, 3, 12, 15, 16]]
        source_country.rename(columns={"index_x": "index", "Country": "Source Country", "Latitude": "Source Latitude", "Longitude": "Source Longitude"}, inplace=True)

        destination_country = destination_country.iloc[:, [0, 3, 12, 15, 16]]
        destination_country.rename(columns={"index_x": "index", "Country": "Destination Country", "Latitude": "Destination Latitude", "Longitude": "Destination Longitude"}, inplace=True)

        internaldf = source_country.merge(destination_country, left_on="index", right_on="index", how="left")

        internaldf.dropna(inplace=True)
        internaldf["internal"] = internaldf["Source Country"] == internaldf["Destination Country"]
        internaldf = internaldf[(internaldf["Source airport"] == airport) | (internaldf["Destination airport"] == airport)]

        if not internaldf.empty:
            if show_internal:
                # Filter only internal flights
                internaldf = internaldf[internaldf["internal"] == True]
                ...
                world = gpd.read_file(gpd.datasets.get_path('naturalearth_lowres'))
                ax = world.plot(figsize=(10, 6), edgecolor='black')

                # Plot airports
                gdf_airports_source = gpd.GeoDataFrame(internaldf, geometry=[Point(lon, lat) for lon, lat in zip(internaldf['Source Longitude'], internaldf['Source Latitude'])])
                gdf_airports_source.plot(ax=ax, marker='o', color='red', markersize=50, label='Source Airports')

                gdf_airports_dest = gpd.GeoDataFrame(internaldf, geometry=[Point(lon, lat) for lon, lat in zip(internaldf['Destination Longitude'], internaldf['Destination Latitude'])])
                gdf_airports_dest.plot(ax=ax, marker='o', color='blue', markersize=50, label='Destination Airports')

                # Plot flights
                gdf_flights = gpd.GeoDataFrame(internaldf, geometry=internaldf['flight_geometry'])
                gdf_flights.plot(ax=ax, color='green', linewidth=2, linestyle='dashed', label='Flights')
    ...
    def plot_most_used_airplane_models(self, countries=None, top_n=None):
        ...
        # Use errors='coerce' to set any problematic values to NaN, which can then be handled appropriately
        self.routes['Source airport ID'] = pd.to_numeric(self.routes['Source airport ID'], errors='coerce').fillna(0).astype(int)
        self.routes['Destination airport ID'] = pd.to_numeric(self.routes['Destination airport ID'], errors='coerce').fillna(0).astype(int)
        ...
        if countries is not None:
            ...
            country_airports = self.airports[self.airports['Country'].isin(countries)]['Airport ID'].unique()

            # Filter routes where either the source or destination airport ID is in the specified countries
            filtered_routes = self.routes[(self.routes['Source airport ID'].isin(country_airports)) |
                                  (self.routes['Destination airport ID'].isin(country_airports))]
        else:
            filtered_routes = self.routes

        # Modify this line to handle None value for top_n
        equipment_counts = filtered_routes['Equipment'].str.split(expand=True).stack().value_counts()
        ...
        plt.figure(figsize=(10, 6))
        sns.barplot(x=equipment_counts.values, y=equipment_counts.index, palette='viridis')
        plt.title(f'Top {top_n} Most Used Airplane Models')
        plt.xlabel('Number of Routes')
        plt.ylabel('Equipment (Airplane Model)')
        plt.show()        
 
    def internal_country2(self, country, show_internal=True):
        """
        ...
        """
        source = self.routes[["index", "Airline", "Airline ID", "Source airport", "Source airport ID", "Codeshare", "Stops", "Equipment"]]
        destination = self.routes[["index", "Airline", "Airline ID", "Destination airport", "Destination airport ID", "Codeshare", "Stops", "Equipment"]]

        source_country = source.merge(self.airports, left_on="Source airport", right_on="IATA", how="left")
        destination_country = destination.merge(self.airports, left_on="Destination airport", right_on="IATA", how="left")
        ...
        internal_df = source_country.merge(destination_country, on="index", how="left")
        ...
        if not internal_df.empty:
            if show_internal:
                # Filter only internal flights
                internal_df = internal_df[internal_df["internal"]==True]
                ...
                # Plot airports
                gdf_airports = gpd.GeoDataFrame(internal_df, geometry=[Point(lon, lat) for lon, lat in zip(internal_df['Source Longitude'], internal_df['Source Latitude'])])
                gdf_airports.plot(axes=axes, marker='o', color='red', markersize=50, label='Airports')

                # Plot flights
                gdf_flights = gpd.GeoDataFrame(internal_df, geometry=internal_df['flight_geometry'])
                gdf_flights.plot(axes=axes, color='blue', linewidth=2, linestyle='dashed', label='Flights')
    ...
    def aircraft_info(self, aircraft_name: str):
        ...
        try:
            ...
            if aircraft_info.empty:
                ...
                print(f"Aircraft model '{aircraft_name}' not found in the data.")
            
                # Fuzzy string matching to find similar aircraft names
                similar_names = process.extract(aircraft_name, available_aircrafts, limit=5)
                similar_names = [(name, score) for name, score in similar_names if score > 50]
            
                if not similar_names:
                    print("No similar aircraft models found. Here is the list of all available aircraft models:")
            ...
            result = llm.invoke(f'What can you tell us about this aircraft model (in a table of specifications): {aircraft_name}')
    ...
    def airport_info(self, airport_name: str):
        ...
        try:
            ...
            if airport_info.empty:
                ...
                similar_names = process.extract(airport_name, available_airports, limit=5)
                similar_names = [(name, score) for name, score in similar_names if score > 50]
        
                if not similar_names:
                    print("No similar airports found. Here is the list of all available airports:")
            ...
            llm = ChatOpenAI(temperature=0.1)
            result = llm.invoke(f'What can you tell us about this airport  (in a table of specifications): {airport_name}')
            display(Markdown(result.content))   
